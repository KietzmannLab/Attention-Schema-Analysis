...
class training_without_as:
    def __init__(
        ...
        actions: int=3,
        random_schema_action: bool = False,
        fixed_attention_schema: bool = False,
        ...
        self.train_agent_without_as = Train(
			...
			noise=self.noise,
            random_schema_action=self.random_schema_action,
            fixed_attention_schema=self.fixed_attention_schema,
            actions=self.actions
                )
            
        self.train_agent_without_as_env = tf_py_environment.TFPyEnvironment(
            self.train_agent_without_as
        )
        
        
        self.test_agent_without_as_attention_score = Train(
        ...
        self.test_agent_without_as_attention_score_env = tf_py_environment.TFPyEnvironment(
            self.test_agent_without_as_attention_score
        )
        self.test_agent_without_as_catch_score = Train(
        ...
        self.test_agent_without_as_catch_score_env = tf_py_environment.TFPyEnvironment(
            self.test_agent_without_as_catch_score
        )
        ...
        self.test_attention_score_fixed_schema = Train(
			attention_reward = True,
			catch_reward = False,
			noise_removing = self.noise_removing,
			attention_schema=self.attention_schema,
			...
			noise=self.noise,
            random_schema_action=self.random_schema_action,
            fixed_attention_schema=True,
            actions=self.actions			)
        
        self.test_attention_score_fixed_schema_env = tf_py_environment.TFPyEnvironment(
			self.test_attention_score_fixed_schema
		)
		
        self.test_catch_score_fixed_schema = Train(
			attention_reward = False,
			catch_reward = True,
			noise_removing = self.noise_removing,
			attention_schema=self.attention_schema,
        ...
        self.test_catch_score_fixed_schema_env = tf_py_environment.TFPyEnvironment(self.test_catch_score_fixed_schema)
    ...
    def _validate(self):
            ...
            utils.validate_py_environment(self.train_agent_without_as, episodes=self.episodes)
            ...
            utils.validate_py_environment(self.test_agent_without_as_attention_score, episodes=self.episodes)
            utils.validate_py_environment(self.test_agent_without_as_catch_score, episodes=self.episodes)

    def start_train(self):
        if self.algorithm == "dqn":
            q_net = q_network.QNetwork(
                self.train_agent_without_as_env.time_step_spec().observation,
                self.train_agent_without_as_env.action_spec(),
                fc_layer_params=(
            ...
            agent = dqn_agent.DqnAgent(
                time_step_spec=self.train_agent_without_as_env.time_step_spec(),
                action_spec=self.train_agent_without_as_env.action_spec(),
                q_network=q_net,
            ...
            actor_net = ActorDistributionNetwork(
                input_tensor_spec=self.train_agent_without_as_env.observation_spec(),
                output_tensor_spec=self.train_agent_without_as_env.action_spec(),
        ...
        replay_buffer = tf_uniform_replay_buffer.TFUniformReplayBuffer(
            ...
            batch_size=self.train_agent_without_as_env.batch_size,  # where does the batch size come from??
        ...
        while i < self.training_iters:
            ...
            attention_score_fixed_schema=compute_avg_return(
                    self.test_attention_score_fixed_schema_env, agent.policy, testing_iterations
                )
            
            catch_score_fixed_schema=compute_avg_return(
                    self.test_catch_score_fixed_schema_env, agent.policy, testing_iterations
                )
            
            attention_score_enabled_as=compute_avg_return(
                    self.test_attention_score_env, agent.policy, testing_iterations
                )
            
            catch_score_enabled_as=compute_avg_return(
            ...
            if path.is_file():
                ...
                results_update["attention_score_fixed_schema"] = attention_score_fixed_schema
                results_update["catch_score_fixed_schema"] = catch_score_fixed_schema
                results_update["attention_score_enabled_as"] = attention_score_enabled_as
                results_update["catch_score_enabled_as"] = catch_score_enabled_as
                results_update["loss"] = training_loss
                results_update_dict = pd.DataFrame(results_update, index=[i])
                
                results = pd.concat([results, results_update_dict], ignore_index=True)
            else:
                ...
                results_dict["attention_score_fixed_schema"] = attention_score_fixed_schema
                results_dict["catch_score_fixed_schema"] = catch_score_fixed_schema
                results_dict["attention_score_enabled_as"] = attention_score_enabled_as
                results_dict["catch_score_enabled_as"] = catch_score_enabled_as
